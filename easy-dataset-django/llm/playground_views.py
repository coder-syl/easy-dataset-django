...
@swagger_auto_schema(
    method='post',
    operation_summary='模型测试聊天（全局，不绑定项目）',
    request_body=openapi.Schema(
        type=openapi.TYPE_OBJECT,
        properties={
            'messages': openapi.Schema(type=openapi.TYPE_ARRAY, items=openapi.Schema(type=openapi.TYPE_OBJECT)),
...
@api_view(['POST'])
def playground_chat_global(request):
    """模型测试聊天（全局，不绑定项目）"""
    try:
        messages = request.data.get('messages', [])
        model = request.data.get('model')
...
@swagger_auto_schema(
    method='post',
    operation_summary='模型测试流式聊天（全局，不绑定项目）',
    request_body=openapi.Schema(
        type=openapi.TYPE_OBJECT,
        properties={
            'messages': openapi.Schema(type=openapi.TYPE_ARRAY, items=openapi.Schema(type=openapi.TYPE_OBJECT)),
...
@api_view(['POST'])
def playground_chat_stream_global(request):
    """模型测试流式聊天（全局，不绑定项目），返回 SSE"""
    try:
        messages = request.data.get('messages', [])
        model_config = request.data.get('model')
        temperature = request.data.get('temperature', 0.7)
