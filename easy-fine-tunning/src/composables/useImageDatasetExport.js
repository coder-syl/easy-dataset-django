...
import { exportImageDatasets as exportImageDatasetsAPI } from '@/api/imageDatasets';

/**
 * 解析标签格式的答案
 * 如果答案是 JSON 数组格式，解析并用逗号连接
 */
function parseAnswerLabels(item) {
...
export function useImageDatasetExport(projectId) {
  const { t } = useI18n();
  ...
  const exportImageDatasets = async (exportOptions) => {
    try {
      // 1. 获取数据集数据
      const response = await exportImageDatasetsAPI(projectId.value, {
        confirmedOnly: exportOptions.confirmedOnly,
        format: exportOptions.formatType === 'raw' ? 'json' : 'json'
      });

      let datasets = response?.data || response;

      if (!datasets || datasets.length === 0) {
        ElMessage.warning(t('imageDatasets.noDataToExport', '没有可导出的数据'));
        return false;
      }
      ...
      a.download = `image-datasets-${projectId.value}-${formatSuffix}-${dateStr}.json`;
      ...
      ElMessage.success(t('imageDatasets.exportSuccess', '数据集导出成功'));
      ...
      if (exportOptions.exportImages) {
        try {
          ...
          const zipUrl = `/projects/${projectId.value}/image-datasets/export-zip?${params.toString()}`;
          ...
          ElMessage.success(t('imageDatasets.exportImagesSuccess', '图片压缩包导出成功'));
        } catch (error) {
          console.error('Failed to export images:', error);
          ElMessage.error(t('imageDatasets.exportImagesFailed', '图片导出失败'));
        }
      }
      ...
      ElMessage.error(error.message || t('imageDatasets.exportFailed', '导出失败'));
      return false;
    }
  };
