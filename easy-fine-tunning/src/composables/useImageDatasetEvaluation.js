...
import { evaluateImageDataset, batchEvaluateImageDatasets } from '@/api/imageDatasets';
import { useModelStore } from '@/stores/model';

/**
 * 图像数据集评估相关的 Composable
 * 封装单个评估和批量评估的逻辑
 */
export function useImageDatasetEvaluation(projectId, onEvaluationComplete) {
  ...
  const handleEvaluateDataset = async (dataset) => {
    ...
    try {
      ...
      const result = await evaluateImageDataset(projectIdRef.value, dataset.id, {
        model,
        language: 'zh-CN'
      });
  ...
  const handleBatchEvaluate = async () => {
    ...
    try {
      ...
      const result = await batchEvaluateImageDatasets(projectIdRef.value, {
        model,
        language: 'zh-CN'
      });
      ...
      if (result?.success || result?.data) {
        ...
        router.push(`/projects/${projectIdRef.value}/tasks`);
      } else {
        ElMessage.error(
