...
export default function QuestionListView({
  ...
  const handleGenerateMultiTurnDataset = async (questionId, questionInfo) => {
    try {
      ...
      const configRaw = await configResponse.json();
      // 兼容后端返回 { code, message, data: {...} } 的情况
      const config =
  ...
  const questionsList = Array.isArray(questions) ? questions : Array.isArray(questions?.data) ? questions.data : [];
              ...
              >
                ...
                <Box sx={{ width: 150, mr: 2, display: { xs: 'none', md: 'block' } }}>
                  {(() => {
                    ...
                    const labelText = imageId
                      ? `Image: ${imageName || ''}`
                      : `${t('chunks.title')}: ${chunkName || t('common.unknown')}`;
                    return (
