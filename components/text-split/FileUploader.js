...
export default function FileUploader({
  ...
  const fetchUploadedFiles = async (page = currentPage, size = pageSize, fileName = searchFileName) => {
    try {
      setLoading(true);
      const result = await fileApi.getFiles({ projectId, page, size, fileName, t });
      ...
      setUploadedFiles({
        ...
        pageSize: filesData.pageSize || size
      });

      setIsFirstUpload((filesData.total || 0) === 0);
  ...
  const handleStartUpload = async domainTreeActionType => {
    ...
    try {
      ...
      for (const file of files) {
        ...
        const response = await fileApi.uploadFile({ file, projectId, fileContent, fileName, t });
        
        // 兼容 Django 响应格式: {code: 0, data: {...}} 或直接返回数据
        const data = response.code === 0 ? response.data : response;
        
        // 提取文件名和文件ID
        const file_name = data.file_name || data.fileName || fileName;
        const fileId = data.fileId || data.file_id;
        
        if (!file_name || !fileId) {
          console.error('文件上传响应格式异常:', response);
          throw new Error('文件上传响应格式异常，无法获取文件名或文件ID');
        }
        
        uploadedFileInfos.push({ file_name, fileId });
      }
  ...
  const handleDeleteFile = async domainTreeActionType => {
    ...
    try {
      ...
      toast.success(t('textSplit.deleteSuccess', { fileName: nameForMsg }));
    } catch (error) {
