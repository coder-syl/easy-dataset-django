...
export default function ChunkList({
  ...
  const handleBatchDelete = async () => {
    ...
    try {
      // 调用批量删除API
      const response = await fetch(`/api/projects/${projectId}/chunks/batch-delete`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      const result = await response.json();
      // 处理Django API响应格式: {code: 0, message: "Success", data: {...}}
      const responseData = result?.code === 0 ? result.data : result;
      ...
      if (responseData?.success || result?.success) {
        ...
        // 显示成功消息
        const deletedCount = responseData?.deletedCount || selectedChunks.length;
        toast.success(t('textSplit.batchDeleteSuccess', { count: deletedCount }) || `成功删除 ${deletedCount} 个文本块`);
      } else {
        throw new Error(responseData?.message || result?.message || '批量删除失败');
      }
    } catch (error) {
      console.error('批量删除失败:', error);
      toast.error(error.message || t('textSplit.batchDeleteFailed', { defaultValue: '批量删除失败' }));
    }
  };
  ...
  const handleOpenBatchDelete = () => {
    if (selectedChunks.length === 0) {
      toast.warning(t('textSplit.selectChunksFirst', { defaultValue: '请先选择要删除的文本块' }));
      return;
    }
