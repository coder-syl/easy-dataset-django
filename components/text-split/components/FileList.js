...
export default function FileList({
  ...
  const handleSelectAll = async () => {
    try {
      ...
      // Django 返回格式: {code, message, data: {allFileIds: [...]}}
  ...
  const handleDownload = async (fileId, file_name) => {
    setPageLoading(true);
    const text = await getFileContent(fileId);

    // Modify the filename if it ends with .pdf
    let downloadName = file_name || 'download.txt';
  ...
  const fetchProjectModel = async () => {
    try {
      ...
      const projectData = projectRaw.data || projectRaw;
      ...
      const modelConfigRaw = await modelResponse.json();
      const modelConfigData = modelConfigRaw.data || modelConfigRaw;
  ...
  const openBatchGenDialog = () => {
    // 如果没有选中文件，自动选中所有文件
    if (array.length === 0 && files?.data && Array.isArray(files.data) && files.data.length > 0) {
  ...
  return (
    ...
    >
      ...
      <Box sx={{ mb: 2 }}>
        ...
        <Box
          sx={{
            ...
            alignItems: 'center',
            mb: isFullscreen && (files?.total || 0) > 0 ? 2 : 0
          }}
        >
          <Typography variant="subtitle1">{t('textSplit.uploadedDocuments', { count: files?.total || 0 })}</Typography>

          {/* 批量生成GA对按钮 */}
          {files?.total > 0 && (
            <Box sx={{ display: 'flex', gap: 1 }}>
              {/* 全选/取消全选按钮 */}
              {array.length === (files?.total || 0) ? (
                <Button
                  variant="outlined"
                  size="small"
        ...
        {isFullscreen && ((files?.total || 0) > 0 || searchTerm) && (
          <Box>
            <TextField
              size="small"
                ...
                {searchLoading
                  ...
                  : searchTerm
                    ? t('textSplit.searchResults', {
                        ...
                        total: files?.total || 0,
                        defaultValue: `找到 ${files?.data?.length || 0} 个文件（共 ${files?.total || 0} 个）`
                      })
                    : null}
              </Typography>
            )}
      ...
      {loading ? (
      ...
      ) : !files?.data || !Array.isArray(files.data) || files.data.length === 0 ? (
        <Box sx={{ p: 4, textAlign: 'center' }}>
          <Typography variant="body2" color="textSecondary">
            {searchTerm
              ? // 搜索有结果但当前页没数据
        ...
        <>
          <List
            ...
            dense // 使列表项更紧凑，减少高度
          >
            {files?.data && Array.isArray(files.data) && files.data.map((file, index) => (
              <Box key={index}>
                <ListItem
                  sx={{
                    display: 'flex',
                ...
                >
                  ...
                  >
                    ...
                    <Tooltip title={`${file.file_name}（${t('textSplit.viewDetails')}）`}>
                      <ListItemText
                        ...
                        secondary={
                          <Typography
                            ...
                            component="div"
                          >
                            {`${formatFileSize(file.size)} · ${new Date(file.create_at || file.createAt).toLocaleString()}`}
                          </Typography>
                        }
                      />
                    </Tooltip>
                  ...
                  >
                    ...
                    />
                    <GaPairsIndicator projectId={projectId} fileId={file.id} file_name={file.file_name} />
                    <Tooltip title={t('textSplit.download')}>
                      <IconButton color="primary" onClick={() => handleDownload(file.id, file.file_name)}>
                        <Download />
                      </IconButton>
                    </Tooltip>
                    <Tooltip title={t('textSplit.deleteFile')}>
                      <IconButton color="error" onClick={() => onDeleteFile(file.id, file.file_name)}>
                        <DeleteIcon />
                      </IconButton>
                    </Tooltip>
                  </Box>
          ...
          {(files?.total || 0) > 10 && (
            <Box sx={{ display: 'flex', justifyContent: 'center', mt: 2 }}>
              <Pagination
                count={Math.ceil((files?.total || 0) / 10)} // 假设每页10个文件
                page={currentPage}
        ...
        <DialogActions>
          ...
          {!genResult && (
            <Button
              ...
              variant="contained"
              disabled={generating || array.length === 0 || !(projectModel || selectedModelInfo)}
