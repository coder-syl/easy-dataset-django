...
export default function DomainAnalysis({ projectId, toc = '', loading = false }) {
  ...
  const getTags = async () => {
    ...
    // 处理Django API响应格式: {code: 0, message: "Success", data: {tags: [...]}}
    const responseData = response.data;
    if (responseData?.code === 0 && responseData?.data?.tags) {
      setTags(responseData.data.tags);
    } else if (responseData?.tags) {
      // 兼容旧格式（直接返回tags）
  ...
  const saveTagChanges = async updatedTags => {
    ...
    try {
      ...
      if (!response.ok || (responseData.code !== undefined && responseData.code !== 0)) {
        throw new Error(responseData.message || responseData.error || t('domain.errors.saveFailed'));
      }
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      if (res.status === 200 && (responseData?.code === 0 || !responseData?.code)) {
        toast.success('删除成功');
        getTags();
      } else {
  ...
  // 检查是否有目录结构或标签数据
  // toc 可能是字符串或数组，需要兼容处理
  const hasToc = toc && (typeof toc === 'string' ? toc.trim().length > 0 : Array.isArray(toc) ? toc.length > 0 : false);
              ...
              >
                {hasToc ? (
                  <div className="markdown-body">
                    ...
                    >
                      {typeof toc === 'string' ? toc : Array.isArray(toc) ? toc.join('\n') : ''}
                    </ReactMarkdown>
                  </div>
                ) : (
                  <Box sx={{ textAlign: 'center', py: 4 }}>
