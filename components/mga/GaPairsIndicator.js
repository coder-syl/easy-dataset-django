 ...
 * @param {string} props.file_name - File name for display
 */
export default function GaPairsIndicator({ projectId, fileId, file_name = '未命名文件' }) {
  ...
  return (
    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
      ...
      <Dialog open={detailsOpen} onClose={handleCloseDialog} maxWidth="lg" fullWidth>
        <DialogTitle>GA Pairs for {file_name}</DialogTitle>
        <DialogContent>
          {detailsOpen && (
