  ...
  const fetchPendingTasks = async () => {
    ...
    try {
      ...
      if (response.data?.code === 0) {
        ...
        const tasks = Array.isArray(response.data.data?.data) 
        ...
        if (hasActiveFileTask) {
          ...
          // 解析任务详情信息
          if (activeTask && activeTask.detail) {
            try {
              const detailInfo = typeof activeTask.detail === 'string' ? JSON.parse(activeTask.detail) : activeTask.detail;
              setTask(detailInfo);
            } catch (e) {
