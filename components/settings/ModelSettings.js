...
export default function ModelSettings({ projectId }) {
  ...
  const getProvidersList = () => {
    axios.get('/api/llm/providers').then(response => {
      // Django 响应可能是 {code, message, data: [...]} 或直接数组
      const providersData = Array.isArray(response.data) ? response.data : response.data?.data || [];

      console.log('获取的模型列表:', providersData);

      // 确保是数组
      const safeProviders = Array.isArray(providersData) ? providersData : [];
      setProviderList(safeProviders);

      const providerOptions = safeProviders.map(provider => ({
        id: provider.id,
        label: provider.name
      }));
      ...
      if (safeProviders.length > 0) {
        ...
        if (safeProviders[0]?.id) {
          getProviderModels(safeProviders[0].id);
        }
      } else {
        setSelectedProvider({});
      }
  ...
  const getModelConfigList = () => {
    axios
      ...
      .then(response => {
        ...
        // 规范字段为前端使用的驼峰命名，兼容后端下划线字段
        const normalizedList = Array.isArray(modelConfigList)
          ? modelConfigList.map(item => ({
              ...item,
              id: item.id,
        ...
        setModelConfigList(normalizedList);
        setLoading(false);
      })
      .catch(error => {
        setLoading(false);
  ...
  const handleSaveModel = () => {
    // 构造后端期望的字段（下划线命名），避免携带 id/create_at 等无关字段
    const payload = {
    ...
    axios
      .post(`/api/projects/${projectId}/model-config`, payload)
      .then(response => {
        const resData = response.data?.data || response.data;
        if (selectedModelInfo && selectedModelInfo.id === resData?.id) {
          setSelectedModelInfo(resData);
        }
  ...
  const getModelStatusInfo = model => {
    ...
    const hasApiKey = model?.apiKey || model?.hasApiKey;
            ...
            >
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <Box sx={{ display: 'flex', gap: 1 }}>
                  {(() => {
                    const providerId = (model?.providerId || '').toString().toLowerCase();
                    const statusInfo = getModelStatusInfo(model);
                    const needsApiKeyNotice = providerId !== 'ollama' && !model?.apiKey;
                    const chipLabel = `${formatEndpoint(model)}${needsApiKeyNotice ? ' (' + t('models.unconfiguredAPIKey') + ')' : ''}`;
                    return (
            ...
            <Grid item xs={12} style={{ display: 'flex', alignItems: 'center' }}>
              <FormControl style={{ width: '70%' }}>
                {(() => {
                  ...
                  const autoCompleteOptions = safeModels
