...
class WorkflowManage:
    def __init__(self, flow: Flow, params, work_flow_post_handler: WorkFlowPostHandler,
                 ...
                 other_list=None,
                 type=None,  # 场景类型参数
                 user_info=None,  # 客户信息参数
                 ip_address=None,  # IP地址参数
                 start_node_id=None,
                 start_node_data=None, chat_record=None, child_node=None):
        logger.info(f"Initializing workflow manager with flow: {flow}, params: {params}")
        ...
        self.type = type  # 保存场景类型
        self.user_info = user_info  # 保存客户信息
        self.ip_address = ip_address  # 保存IP地址
        self.params = params
        self.flow = flow
        self.context = {}
        self.context['ip_address'] = ip_address  # 保存IP地址到context
    ...
    def load_node(self, chat_record, start_node_id, start_node_data):
        ...
        if chat_record is None:
            ...
            # 只有当chat_record不为None且有details时才处理节点详情
            if hasattr(chat_record, 'details') and chat_record.details:
    ...
    def run_chain_manage(self, current_node, node_result_future, language='zh'):
        logger.info(f"Running chain manage for node: {current_node.type if current_node else 'None'}")
        translation.activate(language)
        if current_node is None:
            start_node = self.get_start_node()
            logger.info(f"Got start node: {start_node.type}")
            current_node = get_node(start_node.type)(start_node, self.params, self)
            logger.info(f"Created start node instance: {current_node.type}")
            # 调用valid_args方法初始化序列化器
            current_node.valid_args(current_node.node_params, current_node.workflow_params)
            logger.info(f"Setting type parameter: {self.type}")
            logger.info(f"Setting user_info parameter: {self.user_info}")
            logger.info(f"Workflow params: {self.params}")
        
        # 添加更详细的日志
        logger.info(f"Current node ID: {current_node.id}")
        logger.info(f"Current node params: {current_node.node_params}")
        logger.info(f"Current node properties: {current_node.node.properties}")
        logger.info(f"Workflow params: {self.params}")
        ...
        if result is None:
            logger.warning(f"Chain execution returned None result for node: {current_node.type}")
            return
            
        node_list = self.get_next_node_list(current_node, result)
        logger.info(f"Got next node list: {[node.type for node in node_list]}")
        
        if len(node_list) == 1:
            logger.info(f"Executing single next node: {node_list[0].type}")
            self.run_chain_manage(node_list[0], None, language)
        elif len(node_list) > 1:
            logger.info(f"Executing multiple next nodes: {[node.type for node in node_list]}")
    ...
    def run_chain(self, current_node, node_result_future=None):
        ...
        try:
            ...
            logger.error(f"Error in run_chain for node {current_node.type}: {str(e)}")
            traceback.print_exc()
        return None
    ...
    def run_node(self, node):
        logger.info(f"Executing node: {node.type}")
        try:
            result = node.run()
            logger.info(f"Node {node.type} execution completed successfully")
            return result
        except Exception as e:
            logger.error(f"Error executing node {node.type}: {str(e)}")
            raise
    ...
    def get_next_node_list(self, current_node, current_node_result):
        ...
        @return:  可执行节点列表
        """
        logger.info(f"Getting next node list for node: {current_node.type}")
        # 判断是否中断执行
        ...
        if current_node_result is not None and current_node_result.is_assertion_result():
            logger.info("Processing assertion result")
            for edge in self.flow.edges:
                if (edge.sourceNodeId == current_node.id and
            ...
            logger.info("Processing normal result")
            for edge in self.flow.edges:
                if edge.sourceNodeId == current_node.id:
                    logger.info(f"Found edge from current node to: {edge.targetNodeId}")
                    next_node = [node for node in self.flow.nodes if node.id == edge.targetNodeId]
                    if len(next_node) == 0:
                        continue
                    if next_node[0].properties.get('condition', "AND") == 'AND':
                        ...
                        node_list.append(
                            self.get_node_cls_by_id(edge.targetNodeId,
                                                    [*current_node.up_node_id_list, current_node.node.id]))
        logger.info(f"Returning node list: {[node.type for node in node_list]}")
        return node_list
    ...
    def get_reference_field(self, node_id: str, fields: List[str]):
        ...
        @return:
        """
        logger.info(f"Getting reference field for node_id: {node_id}, fields: {fields}")
        if node_id == 'global':
            result = INode.get_field(self.context, fields)
        else:
            result = self.get_node_by_id(node_id).get_reference_field(fields)
        logger.info(f"Got reference field result: {result}")
        return result
