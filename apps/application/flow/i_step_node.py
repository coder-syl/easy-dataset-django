...
class WorkFlowPostHandler:
    ...
    def handler(self, chat_id,
        ...
        ip_address = workflow.context.get('ip_address', None)
        self.chat_info.append_chat_record(chat_record, self.client_id, asker, ip_address)
        # 重新设置缓存
...
class FlowParamsSerializer(serializers.Serializer):
    ...
    type = serializers.CharField(required=False, default='通用', error_messages=ErrMessage.char("场景类型"))
    user_info = serializers.DictField(required=False, allow_null=True, error_messages=ErrMessage.dict("客户信息"))
