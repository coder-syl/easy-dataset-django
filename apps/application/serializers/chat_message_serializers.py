...
class ChatInfo:
    ...
    def append_chat_record(self, chat_record: ChatRecord, client_id=None, asker=None, ip_address=None):
        ...
        if self.application.id is not None:
            ...
            if not QuerySet(Chat).filter(id=self.chat_id).exists():
                ...
                Chat(id=self.chat_id, application_id=self.application.id, abstract=chat_record.problem_text[0:1024],
                     client_id=client_id, asker=asker_dict, ip_address=ip_address, update_time=datetime.now()).save()
            else:
...
def get_post_handler(chat_info: ChatInfo):
    class PostHandler(PostResponseHandler):
        ...
        def handler(self,
            ...
            chat_info.append_chat_record(chat_record, client_id, asker=asker, ip_address=ip_address)
            # 重新设置缓存
...
class ChatMessageSerializer(serializers.Serializer):
    ...
    type = serializers.CharField(required=False, default='通用', error_messages=ErrMessage.char(_("Scene Type")))
    user_info = serializers.DictField(required=False, allow_null=True, error_messages=ErrMessage.dict(_("User Information")))
    ip_address = serializers.CharField(required=False, allow_null=True, allow_blank=True, error_messages=ErrMessage.char(_("IP Address")))
    ...
    def chat_work_flow(self, chat_info: ChatInfo, base_to_response):
        ...
        work_flow_manage = WorkflowManage(Flow.new_instance(chat_info.work_flow_version.work_flow),
                                          {'history_chat_record': history_chat_record, 'question': message,
                                           ...
                                           'client_type': client_type,
                                           'user_id': user_id,
                                           'type': self.data.get('type', '通用'),
                                           'user_info': self.data.get('user_info', {})}, WorkFlowPostHandler(chat_info, client_id, client_type),
                                          base_to_response, form_data, image_list, document_list, audio_list, other_list,
                                          self.data.get('type', '通用'),  # 传递场景类型参数
                                          self.data.get('user_info', {}),  # 传递客户信息参数
                                          self.data.get('ip_address'),  # 传递IP地址参数
