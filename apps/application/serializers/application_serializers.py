...
class ApplicationSerializer(serializers.Serializer):
    ...
    class PublicQuery(serializers.Serializer):
        """公开应用查询序列化器"""
        name = serializers.CharField(required=False, allow_blank=True, error_messages=ErrMessage.char(_("Application Name")))
        ...
        def get_query_set(self):
            """获取公开应用的查询集"""
            # 获取有发布版本的应用ID列表
            published_app_ids = QuerySet(WorkFlowVersion).filter(
                publish_user_id__isnull=False
            ).values_list('application_id', flat=True).distinct()
            
            query_set = QuerySet(Application).filter(
                id__in=published_app_ids
        ...
        def list(self, with_valid=True):
            ...
            for app in records:
                ...
                # 构建访问地址
                access_url = None
                if application_access_token and application_access_token.is_active:
                    # 构建完整的访问地址，直接使用access_token作为路径参数
                    access_url = f"/ui/chat/{application_access_token.access_token}"
                ...
                app_data = {
                    ...
                    'username': getattr(app.user, 'username', '') if hasattr(app, 'user') and app.user else '',
                    'access_url': access_url  # 添加访问地址
                }
        ...
        def get_response_body_api():
            ...
            return openapi.Schema(
                ...
                required=['total', 'current_page', 'page_size', 'records'],
                properties={
                    'total': openapi.Schema(type=openapi.TYPE_INTEGER, title=_("Total"), description=_("Total number of applications")),
                    'current_page': openapi.Schema(type=openapi.TYPE_INTEGER, title=_("Current Page"), description=_("Current page number")),
                    'page_size': openapi.Schema(type=openapi.TYPE_INTEGER, title=_("Page Size"), description=_("Number of items per page")),
                    'records': openapi.Schema(
                        type=openapi.TYPE_ARRAY,
                        title=_("Records"),
                        description=_("List of public applications"),
                        items=openapi.Schema(
                            type=openapi.TYPE_OBJECT,
                            properties={
                                'id': openapi.Schema(type=openapi.TYPE_STRING, title=_("ID"), description=_("Application ID")),
                                'name': openapi.Schema(type=openapi.TYPE_STRING, title=_("Name"), description=_("Application name")),
                                'desc': openapi.Schema(type=openapi.TYPE_STRING, title=_("Description"), description=_("Application description")),
                                'icon': openapi.Schema(type=openapi.TYPE_STRING, title=_("Icon"), description=_("Application icon")),
                                'type': openapi.Schema(type=openapi.TYPE_STRING, title=_("Type"), description=_("Application type")),
                                'create_time': openapi.Schema(type=openapi.TYPE_STRING, title=_("Create Time"), description=_("Creation time")),
                                'username': openapi.Schema(type=openapi.TYPE_STRING, title=_("Username"), description=_("Creator username")),
                                'access_url': openapi.Schema(type=openapi.TYPE_STRING, title=_("Access URL"), description=_("Public access URL"))
                            }
                        )
                    )
                }
