...
class Application(APIView):
    ...
    class PublicList(APIView):
        ...
        @action(methods=['GET'], detail=False)
        @swagger_auto_schema(operation_summary=_("Get all public applications"),
                             operation_id=_("Get all public applications"),
                             manual_parameters=[
                                 openapi.Parameter(name='current_page', in_=openapi.IN_QUERY, type=openapi.TYPE_INTEGER,
                                                 description='当前页码', default=1),
                                 openapi.Parameter(name='page_size', in_=openapi.IN_QUERY, type=openapi.TYPE_INTEGER,
                                                 description='每页数量', default=20),
                                 openapi.Parameter(name='name', in_=openapi.IN_QUERY, type=openapi.TYPE_STRING,
                                                 description='应用名称搜索'),
                             ],
                             responses=result.get_api_array_response(ApplicationSerializer.PublicQuery.get_response_body_api()),
                             tags=[_('Application')])
        def get(self, request: Request):
            """获取所有公开的应用列表"""
            ...
            return result.success(
                ApplicationSerializer.PublicQuery(
                    data={'current_page': current_page, 'page_size': page_size, 'name': name}).list())
