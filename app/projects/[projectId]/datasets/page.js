...
export default function DatasetsPage({ params }) {
  ...
  const getDatasetsList = async () => {
    try {
      ...
      const list = respData?.data || respData?.datasets || [];
      const total = respData?.total || respData?.count || list.length || 0;
      setDatasets({ data: list, total });
    } catch (error) {
  ...
  const handleSelectAll = async event => {
    if (event.target.checked) {
      ...
      // Django 后端支持 getAllIds=true 返回 {allDatasetIds: [...]}
      let url = `/api/projects/${projectId}/datasets?getAllIds=true`;
      ...
      const response = await axios.get(url);
      // 兼容后端返回 {code, data:{allDatasetIds}} 或直接 {allDatasetIds}
      const respData = response.data?.code === 0 ? response.data?.data : response.data;
      const ids = respData?.allDatasetIds || respData?.map?.(item => item.id) || [];
      setselectedIds(ids);
    } else {
      setselectedIds([]);
    }
