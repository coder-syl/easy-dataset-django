...
export default function TextSplitPage({ params }) {
  ...
  const fetchUploadedFiles = async (page = currentPage, fileName = searchFileName) => {
    try {
      ...
      const params = new URLSearchParams({
        page: page.toString(),
        pageSize: '10'  // 修正参数名：Django 使用 pageSize
      });
      ...
      // Django 返回格式: {code, message, data: {data: [...], total, page, pageSize}}
      // 提取 data 字段
      const result = response.data;
      const filesData = result.data || result;
      setUploadedFiles({
        data: Array.isArray(filesData.data) ? filesData.data : Array.isArray(filesData) ? filesData : [],
        total: filesData.total || 0,
        page: filesData.page || page,
        pageSize: filesData.pageSize || 10
      });
    } catch (error) {
      console.error('Error fetching files:', error);
      toast.error(error.message || '获取文件列表失败');
      setUploadedFiles({ data: [], total: 0 }); // 出错时设置为空数组
    } finally {
      setLoading(false);
    }
  };
  ...
  // 打开删除确认对话框
  const openDeleteConfirm = (fileId, file_name) => {
    setFileToDelete({ fileId, file_name });
  ...
  const confirmDeleteFile = async () => {
    ...
    try {
      ...
      const nameForMsg = fileToDelete.file_name;
      toast.success(t('textSplit.deleteSuccess', { fileName: nameForMsg }) || `删除 ${nameForMsg} 成功`);
    } catch (error) {
  ...
  // 将选中文件传给后端（Django 版要求 fileNames + model）
  const handleSelected = async selectedIds => {
    if (!selectedIds || selectedIds.length === 0) {
      fetchChunks();
      return;
    }

    // 取出文件名列表（Django 期待 fileNames 字段）
    const fileNames = uploadedFiles.data
      .filter(f => selectedIds.includes(String(f.id)))
      .map(f => f.file_name);

    if (fileNames.length === 0) {
      toast.error('未找到选中文件的文件名');
      return;
    }
    ...
    try {
      ...
      const resp = await axios.post(`/api/projects/${projectId}/chunks`, payload);
      const result = resp.data?.data || resp.data;
      const newChunks = result?.chunks || result;
      ...
      console.error('请求分割文本失败:', error);
      toast.error(error?.response?.data?.message || error.message || '分割失败');
    } finally {
      setLoading(false);
    }
  };
        ...
        <DialogContent dividers sx={{ p: 3 }}>
          ...
          <Box sx={{ minHeight: '80vh' }}>
            ...
            <FileList
              ...
              onDeleteFile={(fileId, file_name) => openDeleteConfirm(fileId, file_name)}
              projectId={projectId}
              currentPage={currentPage}
              onPageChange={(page, file_name) => {
                if (file_name !== undefined) {
                  // 搜索时更新搜索关键词和页码
                  setSearchFileName(file_name);
                  setCurrentPage(page);
                } else {
                  // 翻页时只更新页码
                  setCurrentPage(page);
