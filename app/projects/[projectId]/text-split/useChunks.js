  ...
  const fetchChunks = useCallback(
    async (filter = 'all') => {
      try {
        ...
        // 处理Django API响应格式: {code: 0, message: "Success", data: {chunks: [...], toc: '...', tags: [...]}}
        const data = responseData?.code === 0 ? responseData.data : responseData;
        
        setChunks(data.chunks || []);

        // 处理目录结构数据
        // Django API 可能返回 toc 字段，也可能在 fileResult 中
        const tocValue = data.toc || data.fileResult?.toc || '';
        if (tocValue) {
          console.log('TOC数据:', tocValue.substring(0, 100) + '...');
          setTocData(tocValue);
        } else {
          console.log('未找到TOC数据，响应数据:', data);
          // 如果没有toc，设置为空字符串（而不是undefined）
          setTocData('');
        }
      } catch (error) {
