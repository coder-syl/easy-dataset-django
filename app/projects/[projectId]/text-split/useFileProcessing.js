  ...
  const handleFileProcessing = useCallback(
    ...
    async (files, pdfStrategy, selectedViosnModel, domainTreeAction) => {
      ...
      try {
        ...
        // files 可能是对象数组 [{file_name: 'xxx', fileId: 'yyy'}] 或文件名数组 ['xxx']
        const fileList = Array.isArray(files) ? files.map(file => {
          if (typeof file === 'string') {
            return file; // 直接是文件名
          } else if (file && typeof file === 'object') {
            return file.file_name || file.fileName || file.name || ''; // 从对象中提取文件名
          }
          return '';
        }).filter(name => name) : [];

        if (fileList.length === 0) {
          throw new Error('文件列表为空，无法创建处理任务');
        }
