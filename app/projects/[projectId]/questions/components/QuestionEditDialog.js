...
export default function QuestionEditDialog({
  ...
  const getChunks = async projectId => {
    // 获取文本块列表（兼容 Django: {code, data:{chunks}}）
  ...
  useEffect(() => {
    ...
    if (initialData) {
      ...
      const sourceType = initialData.imageId || initialData.image_id ? 'image' : 'text';
      setFormData({
        id: initialData.id,
        question: initialData.question || '',
        sourceType: sourceType,
        chunkId: initialData.chunkId || initialData.chunk_id || '',
        imageId: initialData.imageId || initialData.image_id || '',
