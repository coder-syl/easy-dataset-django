...
export async function GET(request, { params }) {
  try {
    ...
    // 兼容 Django 返回的下划线字段，转换为前端期望的 camelCase
    const normalizeFile = file => ({
      ...
      size: file.size,
      md5: file.md5,
      createAt: file.createAt || file.create_at,
      updateAt: file.updateAt || file.update_at
    });

    const normalized = Array.isArray(files.data) ? files.data.map(normalizeFile) : [];

    return NextResponse.json({
      data: normalized,
      total: files.total || 0,
      page: files.page || page,
      pageSize: files.pageSize || pageSize
    });
  } catch (error) {
